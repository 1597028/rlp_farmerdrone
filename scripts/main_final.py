            ...
            if temp > 0:
                ...
                for x in range(1, 5):
                    if (temp > ((mean_pixels - llindar) * x) and temp < ((mean_pixels + llindar)*x)):
                        ...
                        print(f"La/s vaca/s está/n cerca de las siguientes coordenadas: x={dron_pos_x}, y={dron_pos_y}")
            ...
            if (temp > 0):
                ...
                for x in range(1, 5):
                    if (temp > ((mean_pixels * x) - llindar) and temp < ((mean_pixels * x) + llindar)):
                        ...
                        dron_pos_x, dron_pos_y, _ = get_drone_pos(clientID, dron_handle)
                        print(f"Lo/s cerdo/s está/n cerca de las siguientes coordenadas: x={dron_pos_x}, y={dron_pos_y}")
                        animales_escapados[0] += x
                        print()
...
if clientID != -1:
    ...
    if (option == 2 or option == 3):
        if (animales_escapados[0] != 0):
            #print("No se han podido encontrar todos los animales.\nHa/n quedado ", animales_escapados[0], " animal/es suelto/s :-(")
            print("Se ha/n encontrado", str(animales_escapados[0]), "animal/es escapado/s")
        else:
            print("No han encontrado animales escapados")
