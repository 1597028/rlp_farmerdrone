...
if clientID != -1:
    ...
    _, plano_handle = sim.simxGetObjectHandle(clientID, plano_nombre, sim.simx_opmode_blocking)
    ...
    # Obtener identificador del dron
    _, dron_handle = sim.simxGetObjectHandle(clientID, dron_nombre, sim.simx_opmode_blocking)
    ...
    for z in new_z:
        time.sleep(0.05)
        sim.simxSetObjectPosition(clientID, dron_handle, -1, [dron_pos_x, dron_pos_y, z], sim.simx_opmode_blocking)
    dron_pos_z = dron_height

    while dron_pos_y < maxY:
        new_x = list(np.arange(dron_pos_x, maxX, step=x_step))
        new_y = list(np.arange(dron_pos_y, dron_pos_y + y_movement, step=y_step))
        if dron_pos_x < (maxX - threshold):  # Ida dron
            ...
            angle_rot = list(np.arange(orientation, math.radians(90), step=0.05))
            dron_pos_x, dron_pos_y, _ = get_drone_pos(clientID, dron_handle)
            # Llega final x rotar izquierda
            for angle in angle_rot:
                time.sleep(0.05)
                sim.simxSetObjectOrientation(clientID, dron_handle, -1, [0, 0, angle], sim.simx_opmode_blocking)
            ...
            orientation = get_drone_orientation(clientID, dron_handle)
            angle_rot = list(np.arange(orientation, math.radians(180), step=0.05))
        ...
        if dron_pos_x > (maxX - threshold):
            ...
            orientation = get_drone_orientation(clientID, dron_handle)
            angle_rot = list(np.arange(orientation, math.radians(0), step=0.05))
